...
type WXPayNotifyReq struct {
	...
	Coupon_fee_n   string `xml:"coupon_fee_$n"`
...
type BusFunc func(*APIHandler, *WXPayNotifyReq)

type HandlersMap map[string]*APIHandler
...
func WxpayCallback(w http.ResponseWriter, r *http.Request) {
	...
	//reqMap["sign"] = mr.Sign
	reqMap["err_code"] = mr.Err_code
	reqMap["err_code_des"] = mr.Err_code_des
	...
	if mr.Coupon_count != 0 {
		reqMap["coupon_count"] = mr.Coupon_count
		reqMap["coupon_id_$n"] = mr.Coupon_id_n
		reqMap["coupon_fee_$n"] = mr.Coupon_fee_n
		reqMap["coupon_fee"] = mr.Coupon_fee
		reqMap["coupon_type_$n"] = mr.Coupon_type_n
	}
	...
	if h := handles[mr.Out_trade_no]; h != nil {
		//进行签名校验
		if wxpayVerifySign(reqMap, h.PayKey, mr.Sign) {
			//这里就可以更新我们的后台数据库了，其他业务逻辑同理。
			h.Busfunc(h, &mr)
			delete(handles, mr.Out_trade_no)
			mr.Return_code = "SUCCESS"
			mr.Return_msg = "OK"
		} else {
			mr.Return_code = "FAIL"
			mr.Return_msg = "failed to verify sign, please retry!"
		}
	} else {
		mr.Return_code = "FAIL"
		mr.Return_msg = "处理逻辑不存在，或者已经处理过"
	}
	//结果返回，微信要求如果成功需要返回return_code "SUCCESS"
	bytes, _err := xml.Marshal(mr)
